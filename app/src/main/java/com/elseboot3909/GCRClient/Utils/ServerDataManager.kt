...
class ServerDataManager {
    ...
    companion object {
        ...
        fun writeServerDataList(context: Context) {
            val jsonArray = gson.toJsonTree(serverDataList, object : TypeToken<MutableList<ServerData>>() {}.type)
        ...
        fun loadServerDataList(context: Context) {
            ...
            if (getSharedPreferences(context, mValue) != null && getSharedPreferences(context, mValue)?.getString(mValue, "")?.isNotEmpty() == true) {
                try {
                    serverDataList = gson.fromJson(getSharedPreferences(context, mValue)?.getString(mValue, ""), object : TypeToken<MutableList<ServerData>>() {}.type)
                } catch (ignored: JsonSyntaxException) {
                    serverDataList.clear()
                    serverDataList.add(gson.fromJson(getSharedPreferences(context, mValue)?.getString(mValue, ""), ServerData::class.java))
                }
            } else {
                serverDataList = ArrayList<ServerData>()
            }
        }
        ...
        fun loadSavedPosition(context: Context) {
            selectedPos = getSharedPreferences(context, Constants.SELECTED_SERVER)?.getInt(Constants.SELECTED_SERVER, -1) ?: -1
        }
